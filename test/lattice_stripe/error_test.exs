defmodule LatticeStripe.ErrorTest do
  ...
  describe "test clock error types (Phase 13)" do
    test "constructs a :test_clock_timeout error with raw_body context" do
      ...
      assert err.raw_body["attempts"] == 8
      assert err.raw_body["elapsed_ms"] == 60_142
    end
    ...
    test "constructs a :test_clock_failed error" do
      ...
      assert err.raw_body["last_status"] == "internal_failure"
    end

    test "raising a test_clock_timeout error produces a sensible Exception message" do
      err = %Error{type: :test_clock_timeout, message: "deadline exceeded"}
      ...
      assert msg =~ "deadline exceeded"
    end

    test "pattern matching on :test_clock_timeout works" do
      err = %Error{type: :test_clock_timeout, message: "x"}

      result =
        case err do
          %Error{type: :test_clock_timeout} -> :matched
          _ -> :no_match
        end
    ...
    test "pattern matching on :test_clock_failed works" do
      err = %Error{type: :test_clock_failed, message: "x"}

      result =
        case err do
          %Error{type: :test_clock_failed} -> :matched
          _ -> :no_match
        end
    ...
    test "test_clock atoms are NOT mapped by parse_type/1 (locally-constructed only)" do
      # parse_type/1 only sees string types from HTTP responses. Stripe never
      # sends "test_clock_timeout" or "test_clock_failed" as a :type field, so
      # an unknown type string falls through to :api_error.
      body = %{"error" => %{"type" => "test_clock_timeout", "message" => "x"}}
      error = Error.from_response(500, body, "req_tc")
      assert error.type == :api_error
    end
  end
