defmodule LatticeStripe.Billing.MeterEventSummaryTest do
  ...
  # The three filters Stripe marks `required: true`. Every happy-path call
  ...
  describe "MeterEventSummary.list/4" do
    ...
    # timestamps is exactly what the client refuses to send.
    ...
    # Empty edge: a meter with no usage in the window is the normal state of
    ...
    test "preserves Stripe's wire ordering exactly" do
      items = [
        MeterEventSummaryFixture.meter_event_summary_json(%{
          "id" => "mtrusg_a",
          "start_time" => 1_753_620_000
        }),
        MeterEventSummaryFixture.meter_event_summary_json(%{
          "id" => "mtrusg_b",
          "start_time" => 1_753_706_400
        }),
        MeterEventSummaryFixture.meter_event_summary_json(%{
          "id" => "mtrusg_c",
          "start_time" => 1_753_792_800
        })
      ]

      expect(LatticeStripe.MockTransport, :request, fn _req ->
        ok_response(MeterEventSummaryFixture.meter_event_summary_list_json(items))
      end)
  ...
  describe "MeterEventSummary.from_map/1" do
    test "populates all seven wire fields" do
      assert %MeterEventSummary{
               ...
               meter: "mtr_123",
               livemode: false
             } = MeterEventSummary.from_map(MeterEventSummaryFixture.meter_event_summary_json())
    end

    # `aggregated_value` is a JSON number. Rounding or coercing it to an
    # integer here would silently change a bill.
    test "keeps aggregated_value a float — never rounded, never coerced" do
      summary = MeterEventSummary.from_map(MeterEventSummaryFixture.meter_event_summary_json())
    ...
    test "is idempotent — an already-decoded struct passes through unchanged" do
      once = MeterEventSummary.from_map(MeterEventSummaryFixture.meter_event_summary_json())
  ...
  # the deliberate absence, encoded as design rather than accident
  ...
  # every guard fires BEFORE any transport call.
  #
  ...
  describe "pre-network guards" do
    ...
    # "requires an event_name param". The message format is a verbatim lock.
    ...
    # The second message set must name the function and arity
  ...
  describe "MeterEventSummary.stream!/4" do
    test "yields decoded %MeterEventSummary{} structs across a single page" do
      items = [
        MeterEventSummaryFixture.meter_event_summary_json(%{"id" => "mtrusg_a"}),
        MeterEventSummaryFixture.meter_event_summary_json(%{"id" => "mtrusg_b"})
      ]
  ...
  # structural surface lock. With no Dialyzer and documentation-only
  # typespecs, `refute function_exported?` is the ONLY enforcement of public
  # surface shape in this project.

  describe "module surface" do
    # there is exactly one path, `GET /v1/billing/meters/:id/event_summaries`.
    ...
    # Refuted at arities 1, 2, and 3, not only at the top arity: with two defaulted
    # arguments a lower arity could otherwise slip through unnoticed.
    ...
    # Encodes the rejection structurally. This library will not choose floor
