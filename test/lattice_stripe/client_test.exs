defmodule LatticeStripe.ClientTest do
  ...
  describe "idempotency_key_prefix option (A-13client)" do
    ...
    test "accepts a string prefix" do
      client = test_client(idempotency_key_prefix: "lattice-test-42-")
      assert client.idempotency_key_prefix == "lattice-test-42-"
    end

    test "rejects non-string non-nil prefix" do
      assert_raise NimbleOptions.ValidationError, fn ->
        test_client(idempotency_key_prefix: 42)
      end
    end

    test "auto-generated POST idempotency key starts with configured prefix" do
      client = test_client(idempotency_key_prefix: "lattice-test-42-")

      expect(LatticeStripe.MockTransport, :request, fn req_map ->
        idk = Enum.find(req_map.headers, fn {k, _} -> k == "idempotency-key" end)
        assert idk != nil
        {_, key} = idk
        assert String.starts_with?(key, "lattice-test-42-")
        # Suffix is a uuid4
        suffix = String.replace_prefix(key, "lattice-test-42-", "")
    ...
    test "auto-generated POST idempotency key falls back to idk_ltc_ when prefix is nil" do
      ...
      expect(LatticeStripe.MockTransport, :request, fn req_map ->
        ...
        assert idk != nil
        {_, key} = idk
        assert String.starts_with?(key, "idk_ltc_")
        ok_response()
      end)
    ...
    test "user-supplied opts[:idempotency_key] wins over the prefix" do
      ...
      expect(LatticeStripe.MockTransport, :request, fn req_map ->
        idk = Enum.find(req_map.headers, fn {k, _} -> k == "idempotency-key" end)
        assert idk == {"idempotency-key", "my-explicit-key"}
        ok_response()
      end)

      req = post_request("/v1/charges", %{}, idempotency_key: "my-explicit-key")
      assert {:ok, _} = Client.request(client, req)
    end

    test "GET requests still get no idempotency key even when prefix is set" do
      client = test_client(idempotency_key_prefix: "lattice-test-42-")
