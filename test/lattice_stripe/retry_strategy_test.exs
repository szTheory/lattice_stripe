    ...
    test "attempt 1 has lower average delay than attempt 2" do
      ...
      delays_1 =
        for _ <- 1..50,
            do:
              (
                assert {:retry, d} = RetryStrategy.Default.retry?(1, ctx_base)
                d
              )
      ...
      delays_3 =
        for _ <- 1..50,
            do:
              (
                assert {:retry, d} = RetryStrategy.Default.retry?(3, ctx_base)
                d
              )
    ...
    test "jitter keeps delay between 50-100% of calculated base value" do
      ...
      delays =
        for _ <- 1..100,
            do:
              (
                assert {:retry, d} = RetryStrategy.Default.retry?(1, ctx)
                d
              )
    ...
    test "jitter for attempt 2: delay between 50-100% of 1000ms base" do
      ...
      delays =
        for _ <- 1..100,
            do:
              (
                assert {:retry, d} = RetryStrategy.Default.retry?(2, ctx)
                d
              )
