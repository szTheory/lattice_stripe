defmodule LatticeStripe.InvoiceTest do
  ...
  # The canonical invoice wire shape lives in `lib/` as public API — imported rather than
  # aliased because `LatticeStripe.Invoice` below already owns the `Invoice` alias, and an
  # import cannot collide with it. Every `invoice_json/0,1` call site below is unchanged.
  import LatticeStripe.Testing.Fixtures.Invoice
