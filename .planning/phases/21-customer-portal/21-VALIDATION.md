...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 21-01-T1 | 21-01 | 0 | TEST-04 | T-21-02 | Probe documents sub-field gap; fails closed on bad cases | probe | `elixir scripts/verify_portal_endpoint.exs` | ✅ | ✅ green |
| 21-01-T2 | 21-01 | 0 | TEST-02 | T-21-01 | Fixture URLs are fake placeholders, not bearer creds | compile | `mix compile --warnings-as-errors` | ✅ | ✅ green |
| 21-01-T3 | 21-01 | 0 | TEST-02 | T-21-01 | Test skeletons compile green before implementation | unit | `mix test test/lattice_stripe/billing_portal/ test/integration/billing_portal_session_integration_test.exs --include billing_portal --exclude integration` | ✅ | ✅ green |
...
| 21-03-T1 | 21-03 | 2 | PORTAL-04 | T-21-06 | All 10 guard cases raise pre-network with actionable message; unknown-type binary catchall structurally blocks silent forwarding | unit (tdd) | `mix test test/lattice_stripe/billing_portal/guards_test.exs` | ✅ | ⬜ pending |
...
*Populated by plan 21-01 executor. Status: ⬜ pending · ✅ green · ❌ red · ⚠️ flaky*
*Wave 0 rows (21-01-T1..T3) flipped to ✅ on plan 21-01 completion.*

---

## Wave 0 Requirements

- [x] `scripts/verify_portal_endpoint.exs` — stripe-mock probe covering TEST-04 sub-field gap
- [x] `test/support/fixtures/billing_portal.ex` — `LatticeStripe.Test.Fixtures.BillingPortal.Session` with 5 builders (TEST-02)
- [x] `test/lattice_stripe/billing_portal/session_test.exs` — unit stubs for PORTAL-01/02/05/06 + Inspect
- [x] `test/lattice_stripe/billing_portal/guards_test.exs` — unit stubs for PORTAL-04 10-case matrix
- [x] `test/lattice_stripe/billing_portal/session/flow_data_test.exs` — unit stubs for PORTAL-03 decode cases
- [x] `test/integration/billing_portal_session_integration_test.exs` — stripe-mock integration stub (TEST-05 portal)

*Plan 21-01 landed all six. `wave_0_complete: true` set.*
...
- [x] Wave 0 covers all MISSING references (session_test, guards_test, flow_data_test, integration_test all landed in 21-01)
- [x] No watch-mode flags
- [x] Feedback latency < 30s (portal subset runs in ~5s per Phase 20 baseline)
- [x] `nyquist_compliant: true` set in frontmatter

**Approval:** planner-approved · executor sign-off at Wave 0 completion ✅ (plan 21-01 complete)
